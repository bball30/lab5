    ...
    private int launchCommand(String[] userCommand) {
        ...
        switch (command) {
            ...
            case "remove_any_by_form_of_education":
                if (!commandManager.removeAny(argument)) return 1;
                break;
            case "print_unique_group_admin":
                if (!commandManager.printUniqueGroupAdmin(argument)) return 1;
                break;
            case "print_field_descending_should_be_expelled":
                if (!commandManager.printFieldDescendingShouldBeExpelled(argument)) return 1;
            case "exit":
